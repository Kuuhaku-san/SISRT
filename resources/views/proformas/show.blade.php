...
@section('contenido')
    ...
    <form method="POST" action="/proformas/{{$proforma->codigo}}">
        {{ method_field('PATCH') }}
        {{ csrf_field() }}
                ...
                <table class="table">
                    ...
                    <tbody id="tablaPiezas">
                        ...
                        @foreach ($proforma->piezas as $pieza)
                            <tr class="row item">
                                <td class="col-2">
                                    <input class="form-control cantidad" type="number" name="cantidad[]" min="1" value="{{$pieza->pivot->cantidad}}" required>
                                </td>
                                <td class="col">
                                    <input class="form-control typeahead" type="text" name="nombre_pieza[]" placeholder="Nombre de la pieza" value="{{$pieza->nombre}}" required>
                                </td>
                                <td class="col-3">
                                    <input class="form-control precio" type="number" name="precio[]" min="0.1" step="0.1" value="{{$pieza->pivot->precio}}">
                                </td>
                                @php
                                ...
                                @endphp
                                <td class="col-2">
                                    <input class="form-control total" type="number" value="{{$total}}" readonly>
                                </td>
        ...
        <div class="form-group row">
            ...
            <textarea class="col form-control" id="mano_de_obra" name="mano_de_obra" required rows="8">{{ $proforma->mano_de_obra }}</textarea>
        </div>
        ...
        <div class="form-group row">
            ...
            <input type="number" class="col form-control" id="monto_total" readonly value="{{$proforma->precio_mano_obra + $subtotal}}">
        </div>
